...
import { MASTER_DYNAMICS, masterGainForSpeakers } from "@/lib/audioGainStaging";
...
export type ClubSpeaker = { id: string; kind: SpeakerKind; modelId?: SpeakerModelId; label: string; position: Position3D; orientation?: { yaw: number }; stackParentId?: string | null; stackAlign?: StackAlignment; supportBlockId?: string | null; cabinetColor?: string; level: number; muted: boolean; responseProfileId: string; activity: number; eq: SpeakerEq };
...
type SpeakerCache = SpatialCache & { modelId?: SpeakerModelId; gain?: number; eq: EqCache; orientation?: SpatialCache };
...
function setSpeakerOrientation(node: LegacySpatialNode, yaw: number, cache: SpatialCache, now: number) {
  const forward = speakerOrientationToAudioOrientation(yaw);
  const changed = differs(forward.x, cache.x) || differs(forward.y, cache.y) || differs(forward.z, cache.z);
  if (!changed) return;
  if (node.orientationX && node.orientationY && node.orientationZ) { smoothParam(node.orientationX, forward.x, cache.x, now); smoothParam(node.orientationY, forward.y, cache.y, now); smoothParam(node.orientationZ, forward.z, cache.z, now); }
  else node.setOrientation?.(forward.x, forward.y, forward.z);
  cache.x = forward.x; cache.y = forward.y; cache.z = forward.z;
}
...
export function useClubAudio(speakers: ClubSpeaker[], listener: ClubListener, sources: ClubSource[], activeSourceId: string) {
  ...
  const ensureContext = useCallback(() => { if (contextRef.current) return contextRef.current; const context = new AudioContext(); const master = context.createGain(); const compressor = context.createDynamicsCompressor(); master.gain.value = masterGainForSpeakers(speakersRef.current); masterGainCacheRef.current = master.gain.value; compressor.threshold.value = MASTER_DYNAMICS.thresholdDb; compressor.knee.value = MASTER_DYNAMICS.kneeDb; compressor.ratio.value = MASTER_DYNAMICS.ratio; compressor.attack.value = MASTER_DYNAMICS.attackSeconds; compressor.release.value = MASTER_DYNAMICS.releaseSeconds; master.connect(compressor); compressor.connect(context.destination); contextRef.current = context; masterRef.current = master; return context; }, []);
  ...
  const syncSpeakerDsp = useCallback((speakerList: ClubSpeaker[]) => { const context = contextRef.current; const master = masterRef.current; if (!context || !master) return; const now = context.currentTime; speakerList.forEach((speaker) => { const node = speakerNodesRef.current.get(speaker.id); if (!node) return; syncEq(node, speaker.eq, now); const targetGain = speaker.muted ? 0 : Math.max(.02, speaker.level); smoothParam(node.gain.gain, targetGain, node.cache.gain, now); node.cache.gain = targetGain; }); const targetMasterGain = masterGainForSpeakers(speakerList); smoothParam(master.gain, targetMasterGain, masterGainCacheRef.current, now); masterGainCacheRef.current = targetMasterGain; }, []);
  ...
  const syncSpeakerOrientations = useCallback((speakerList: ClubSpeaker[]) => { const context = contextRef.current; if (!context) return; const now = context.currentTime; speakerList.forEach((speaker) => { const node = speakerNodesRef.current.get(speaker.id); if (node) setSpeakerOrientation(node.panner as unknown as LegacySpatialNode, speaker.orientation?.yaw ?? 0, node.cache.orientation ?? (node.cache.orientation = {}), now); }); }, []);
  ...
  useEffect(() => { syncSpeakerOrientations(speakersRef.current); }, [speakerOrientationKey, syncSpeakerOrientations]);
  ...
  const togglePlayback = useCallback(async () => {
    ...
    try {
      ...
      syncTopology(true); syncSpeakerDsp(speakersRef.current); syncSpeakerPositions(speakersRef.current); syncSpeakerOrientations(speakersRef.current); syncListenerPosition(listenerRef.current); syncListenerOrientation(listenerRef.current);
