/**
 * Sound System Playground audio engine: one chosen song is shaped by every
 * virtual speaker, then placed around the listener for headphones.
 */
...
function makeOfficialVoice(context: AudioContext, source: ClubSource): Voice {
  const output = context.createGain(); output.gain.value = 0.24;
  ...
  lowpass.type = "lowpass"; lowpass.frequency.value = source.id === "pulse" ? 380 : 1150; lowpass.Q.value = 0.4;
  lfo.frequency.value = source.id === "pulse" ? 1.7 : 0.16; lfoGain.gain.value = source.id === "pulse" ? 120 : 26;
...
export function useClubAudio(speakers: ClubSpeaker[], listener: { x: number; y: number }, sources: ClubSource[], activeSourceId: string) {
  const contextRef = useRef<AudioContext | null>(null); const masterRef = useRef<GainNode | null>(null);
  const voicesRef = useRef(new Map<string, Voice>()); const speakerNodesRef = useRef(new Map<string, SpeakerNode>());
  ...
  const ensureContext = useCallback(() => {
    ...
    master.gain.value = 0.82; compressor.threshold.value = -18; compressor.ratio.value = 5; master.connect(compressor); compressor.connect(context.destination);
  ...
  const sync = useCallback(() => {
    ...
    for (const [id, node] of Array.from(speakerNodes.entries())) if (!speakers.some((speaker) => speaker.id === id)) { node.filter.disconnect(); node.gain.disconnect(); node.panner.disconnect(); speakerNodes.delete(id); }
    speakers.forEach((speaker) => {
      let node = speakerNodes.get(speaker.id);
      if (!node) { const filter = context.createBiquadFilter(); const gain = context.createGain(); const panner = context.createPanner(); panner.panningModel = "HRTF"; panner.distanceModel = "inverse"; panner.refDistance = 1.2; panner.maxDistance = 12; panner.rolloffFactor = 0.85; filter.connect(gain); gain.connect(panner); panner.connect(master); node = { filter, gain, panner }; speakerNodes.set(speaker.id, node); }
      const config = filterForKind[speaker.kind]; node.filter.type = config.type; node.filter.frequency.setTargetAtTime(config.frequency, context.currentTime, 0.04); node.filter.Q.setTargetAtTime(config.q, context.currentTime, 0.04);
      node.gain.gain.setTargetAtTime(speaker.muted ? 0 : Math.max(0.02, speaker.level), context.currentTime, 0.04);
      node.panner.positionX.setTargetAtTime((speaker.x - 0.5) * 9, context.currentTime, 0.04); node.panner.positionY.setTargetAtTime(0, context.currentTime, 0.04); node.panner.positionZ.setTargetAtTime((speaker.y - 0.5) * 9, context.currentTime, 0.04);
    });
    context.listener.positionX.setTargetAtTime((listener.x - 0.5) * 8, context.currentTime, 0.04); context.listener.positionY.setTargetAtTime(0, context.currentTime, 0.04); context.listener.positionZ.setTargetAtTime((listener.y - 0.5) * 8, context.currentTime, 0.04);
    const desiredSource = sources.find((source) => source.id === activeSourceId);
    const voices = voicesRef.current;
    for (const [id, voice] of Array.from(voices.entries())) if (id !== activeSourceId || !desiredSource) { voice.stop?.(); voice.media?.pause(); voices.delete(id); }
    if (desiredSource && !voices.has(desiredSource.id)) {
      if (desiredSource.category === "local" && desiredSource.localUrl) { const media = new Audio(desiredSource.localUrl); media.loop = true; media.preload = "auto"; const mediaSource = context.createMediaElementSource(media); const output = context.createGain(); output.gain.value = 0.6; mediaSource.connect(output); voices.set(desiredSource.id, { output, media }); if (context.state === "running") void media.play(); }
      else voices.set(desiredSource.id, makeOfficialVoice(context, desiredSource));
    }
    voices.forEach((voice) => { voice.output.disconnect(); speakers.forEach((speaker) => { const destination = speakerNodes.get(speaker.id); if (destination) voice.output.connect(destination.filter); }); });
  }, [activeSourceId, listener.x, listener.y, sources, speakers]);
