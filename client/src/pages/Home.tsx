...
const speakerMeta: Record<SpeakerKind, { label: string; short: string; color: string; note: string }> = {
  sub: { label: "SUB", short: "S", color: "#2b2d2b", note: "low" }, woofer: { label: "WOOFER", short: "W", color: "#6f726d", note: "warm" },
  full: { label: "FULL RANGE", short: "F", color: "#f1f0e9", note: "full" }, mid: { label: "MID", short: "M", color: "#e0b13c", note: "voice" }, high: { label: "HIGH", short: "H", color: "#4bbd92", note: "air" },
};
const initialSpeakers: ClubSpeaker[] = [
  { id: "sub-1", kind: "sub", label: "SUB", x: .1667, y: .75, level: .78, muted: false }, { id: "full-1", kind: "full", label: "FULL RANGE", x: .3333, y: .3333, level: .72, muted: false },
  { id: "full-2", kind: "full", label: "FULL RANGE", x: .6667, y: .3333, level: .72, muted: false }, { id: "high-1", kind: "high", label: "HIGH", x: .8333, y: .6667, level: .6, muted: false },
];
const clubTracks: ClubSource[] = [
  { id: "pulse", name: "Deep Pulse", category: "official", color: "#e65b4a" }, { id: "rain", name: "Rain Room", category: "official", color: "#d6aa43" }, { id: "bronze", name: "Bronze Air", category: "official", color: "#4bbd92" },
];
const GRID_STEPS = 12;
...
export default function Home() {
  ...
  const moveOnFloor = (event: React.PointerEvent) => { const drag = dragRef.current; if (!drag || !floorRef.current) return; const rect = floorRef.current.getBoundingClientRect(); const x = clamp((event.clientX - rect.left) / rect.width); const y = clamp((event.clientY - rect.top) / rect.height); if (drag.target === "listener") setListener({ x, y }); else if (drag.id) setSpeakers((now) => now.map((speaker) => speaker.id === drag.id ? { ...speaker, x: snapToGrid(x), y: snapToGrid(y) } : speaker)); };
  const addSpeaker = (kind: SpeakerKind) => { const id = `${kind}-${Date.now()}`; setSpeakers((now) => { const position = gridSpawnPoints[now.length % gridSpawnPoints.length]; return [...now, { id, kind, label: speakerMeta[kind].label, x: position.x, y: position.y, level: .68, muted: false }]; }); setSelectedSpeakerId(id); };
  const addLocalSound = (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; const id = `local-${Date.now()}`; setSources((now) => [...now, { id, name: file.name.replace(/\.[^/.]+$/, ""), category: "local", color: "#5e98c8", localUrl: URL.createObjectURL(file) }]); setSelectedSourceId(id); event.target.value = ""; };
  const updateSpeaker = (update: Partial<ClubSpeaker>) => { if (!selectedSpeaker) return; setSpeakers((now) => now.map((speaker) => speaker.id === selectedSpeaker.id ? { ...speaker, ...update, label: update.kind ? speakerMeta[update.kind].label : speaker.label } : speaker)); };
  const removeSelected = () => { if (!selectedSpeaker || speakers.length <= 1) return; setSpeakers((now) => now.filter((speaker) => speaker.id !== selectedSpeaker.id)); setSelectedSpeakerId(speakers.find((speaker) => speaker.id !== selectedSpeaker.id)?.id ?? ""); };

  return <main className="playground-app">
    <header className="playground-header"><div className="play-brand"><img src={logoMark} alt="Club Craft" /><div><p>SOUND SYSTEM PLAYGROUND</p><h1>CLUB CRAFT</h1></div></div><div className="header-center"><span className="track-led" style={{ backgroundColor: selectedSource?.color }} /><span>{selectedSource?.name ?? "Choose a sound"}</span></div><div className="play-actions"><button className="quiet-button" onClick={() => setShowHelp(true)}><CircleHelp size={16} /> HOW TO PLAY</button><button className="hear-button" onClick={() => void togglePlayback()}>{isPlaying ? <Pause size={17} fill="currentColor" /> : <Play size={17} fill="currentColor" />}{isPlaying ? "PAUSE" : "PLAY"}</button></div></header>
    <section className="playground-shell">
      ...
      <section className="room-section"><div className="room-heading"><div><p className="panel-kicker">2. PLACE YOUR SPEAKERS</p><h2>Move them. Hear the room change.</h2></div><div className={`room-status ${isPlaying ? "playing" : ""}`}><i /> {isPlaying ? "ROOM IS PLAYING" : "READY TO PLAY"}</div></div><div ref={floorRef} className="playground-floor" onPointerMove={moveOnFloor} onPointerUp={() => { dragRef.current = null; }} onPointerCancel={() => { dragRef.current = null; }}><div className="floor-plan" /><div className="room-back-wall" /><div className="room-left-wall" /><div className="room-right-wall" /><div className="stage-plinth"><i /><i /><i /></div><div className="floor-annotation north">N / ROOM AXIS</div><div className="floor-annotation scale">0M&nbsp;&nbsp;&nbsp; 3M&nbsp;&nbsp;&nbsp; 6M</div><div className="stage-origin"><span className="stage-cross" /><b>STAGE</b><small>sound starts here</small></div><div className="song-in-room" style={{ "--song-color": selectedSource?.color } as React.CSSProperties}><span>◉</span><small>{selectedSource?.name}</small></div>{selectedSpeaker && <svg className="playground-signal" aria-hidden="true" viewBox="0 0 100 100" preserveAspectRatio="none"><line x1="50" y1="42" x2={selectedSpeaker.x * 100} y2={selectedSpeaker.y * 100} /><circle cx={selectedSpeaker.x * 100} cy={selectedSpeaker.y * 100} r="1.18" /></svg>}{speakers.map((speaker) => { const meta = speakerMeta[speaker.kind]; return <button key={speaker.id} className={`speaker-puck playground-speaker ${speaker.id === selectedSpeakerId ? "selected" : ""} ${speaker.muted ? "muted" : ""}`} style={{ left: `${speaker.x * 100}%`, top: `${speaker.y * 100}%`, "--speaker-color": meta.color } as React.CSSProperties} onPointerDown={(event) => startDrag(event, "speaker", speaker.id)} onClick={() => setSelectedSpeakerId(speaker.id)}><span className="print-layers" /><span className="cabinet-grille" /><strong>{meta.short}</strong><small>{meta.label}</small></button>; })}<button className="listener-marker playground-listener" style={{ left: `${listener.x * 100}%`, top: `${listener.y * 100}%` }} onPointerDown={(event) => startDrag(event, "listener")}><span>◎</span><small>YOU</small></button><div className="floor-tip">SPEAKERS SNAP TO THE GRID / MOVE ◎ YOU</div></div><div className="speaker-add-row"><span>3. ADD A SPEAKER</span>{(Object.keys(speakerMeta) as SpeakerKind[]).map((kind) => <button key={kind} onClick={() => addSpeaker(kind)}><i style={{ backgroundColor: speakerMeta[kind].color }} />{speakerMeta[kind].label}</button>)}</div></section>
