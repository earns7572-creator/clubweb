...
import { BLOCK_DEPTH_METERS, BLOCK_HEIGHT_METERS, BLOCK_WIDTH_METERS, createBlockResolver, type SupportBlock } from "@/lib/blockSupport";
...
function PovSpeaker({ speaker, activity, bandActivity, centerY, xy }: { speaker: ClubSpeaker; activity: number; bandActivity: SpeakerBandActivityMap[string] | undefined; centerY: number; xy: { x: number; y: number } }) { const [x, , z] = world(xy); return <group position={[x, centerY, z]} rotation={[0, speaker.orientation?.yaw ?? 0, 0]}><SpeakerMiniature kind={speaker.kind} modelId={speaker.modelId} activity={activity} bandActivity={bandActivity} glowStrength={1.45} cabinetColor={speaker.cabinetColor} /></group>; }
function PovWorld({ speakers, blocks, activityBySpeaker, bandActivityBySpeaker, surfaceTone }: { speakers: ClubSpeaker[]; blocks: SupportBlock[]; activityBySpeaker: Readonly<Record<string, number>>; bandActivityBySpeaker: SpeakerBandActivityMap; surfaceTone: SurfaceTone }) { const surface = povPalette[surfaceTone]; const stackResolver = createStackResolver(speakers); const blockResolver = createBlockResolver(blocks); return <><fog attach="fog" args={[surface.fog, 8, 30]} /><ambientLight intensity={surface.ambient} /><directionalLight position={[-4, 8, 3]} intensity={surface.directional} color={surface.light} /><mesh rotation={[-Math.PI / 2, 0, 0]}><planeGeometry args={[28, 22]} /><meshStandardMaterial color={surface.floor} roughness={.96} /></mesh>{blocks.map((block) => { const [x, , z] = world(blockResolver.getXY(block)); return <mesh key={block.id} position={[x, blockResolver.getCenterMeters(block), z]}><boxGeometry args={[BLOCK_WIDTH_METERS, BLOCK_HEIGHT_METERS, BLOCK_DEPTH_METERS]} /><meshStandardMaterial color={block.color ?? "#9b8d78"} roughness={.84} /></mesh>; })}{speakers.map((speaker) => <PovSpeaker key={speaker.id} speaker={speaker} activity={activityBySpeaker[speaker.id] ?? 0} bandActivity={bandActivityBySpeaker[speaker.id]} centerY={stackResolver.getCenterMeters(speaker)} xy={stackResolver.getXY(speaker)} />)}<gridHelper args={[24, 24, surface.gridMajor, surface.gridMinor]} position={[0, .01, 0]} /></>; }
