...
function PovSpeaker({ speaker, activity, centerY, xy }: { speaker: ClubSpeaker; activity: number; centerY: number; xy: { x: number; y: number } }) { const [x, , z] = world(xy); return <group position={[x, centerY, z]} rotation={[0, speaker.orientation?.yaw ?? 0, 0]}><SpeakerMiniature kind={speaker.kind} modelId={speaker.modelId} activity={activity} /></group>; }
