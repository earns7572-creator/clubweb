...
import { roomMetricsForViewport } from "@/lib/roomGeometry";
...
const world = (position: Pick<ClubSpeaker["position"], "x" | "y">, room: { width: number; depth: number }) => [(position.x - .5) * room.width, 0, (position.y - .5) * room.depth] as const;
...
function CameraRig({ listener, vibration, motionPose, onInvalidateReady }: { listener: ClubListener; vibration: number; motionPose: MutableRefObject<DeviceLookPose>; onInvalidateReady: (invalidate: () => void) => void }) {
  const { camera, invalidate, size } = useThree(); const room = roomMetricsForViewport(size.width, size.height);
  ...
  const syncBase = () => {
    ...
    const x = (listener.position.x - .5) * room.width; const y = listenerEarHeightMeters(listener.position.z); const z = (listener.position.y - .5) * room.depth;
  ...
  useEffect(() => { syncBase(); invalidate(); }, [listener, invalidate, room.depth, room.width]);
...
function PovSpeaker({ speaker, activity, bandActivity, centerY, xy, room }: { speaker: ClubSpeaker; activity: number; bandActivity: SpeakerBandActivityMap[string] | undefined; centerY: number; xy: { x: number; y: number }; room: { width: number; depth: number } }) { const [x, , z] = world(xy, room); const ref = useRef<THREE.Group>(null); useEffect(() => { ref.current?.traverse((object) => { if (object instanceof THREE.Mesh) { object.castShadow = true; object.receiveShadow = true; } }); }, []); return <group ref={ref} position={[x, centerY, z]} rotation={[0, speaker.orientation?.yaw ?? 0, 0]}><SpeakerMiniature kind={speaker.kind} modelId={speaker.modelId} activity={activity} bandActivity={bandActivity} glowStrength={1.45} cabinetColor={speaker.cabinetColor} /></group>; }
function PovWorld({ speakers, blocks, activityBySpeaker, bandActivityBySpeaker, surfaceTone }: { speakers: ClubSpeaker[]; blocks: SupportBlock[]; activityBySpeaker: Readonly<Record<string, number>>; bandActivityBySpeaker: SpeakerBandActivityMap; surfaceTone: SurfaceTone }) { const { size } = useThree(); const room = roomMetricsForViewport(size.width, size.height); const surface = povPalette[surfaceTone]; const stackResolver = createStackResolver(speakers, room); const blockResolver = createBlockResolver(blocks); const surfaceWidth = room.width * 1.08; const surfaceDepth = room.depth * 1.1; return <><ambientLight intensity={surface.ambient} /><directionalLight castShadow position={[-4, 13, 3]} intensity={surface.directional} color={surface.light} shadow-mapSize={[1024, 1024]} shadow-camera-left={-10} shadow-camera-right={10} shadow-camera-top={10} shadow-camera-bottom={-10} /><mesh position={[0, -.08, 0]} receiveShadow><boxGeometry args={[surfaceWidth, .16, surfaceDepth]} /><meshStandardMaterial color={surface.edge} roughness={.96} metalness={0} /></mesh><mesh rotation={[-Math.PI / 2, 0, 0]} receiveShadow><planeGeometry args={[surfaceWidth, surfaceDepth]} /><meshStandardMaterial color={surface.floor} roughness={.98} metalness={0} /></mesh><ContactShadows position={[0, .012, 0]} opacity={.16} scale={Math.max(14, room.depth * 1.05)} blur={2.4} far={3.4} color={surface.shadow} frames={1} />{blocks.map((block) => { const [x, , z] = world(blockResolver.getXY(block), room); const dimensions = blockDimensions(block); return <group key={block.id} position={[x, blockResolver.getCenterMeters(block), z]} rotation={[0, block.orientation?.yaw ?? 0, 0]}><mesh castShadow receiveShadow><boxGeometry args={[dimensions.width, dimensions.height, dimensions.depth]} /><meshStandardMaterial color="#858781" roughness={.93} metalness={0} /></mesh><mesh castShadow receiveShadow position={[0, dimensions.height / 2 + .012, 0]}><boxGeometry args={[dimensions.width * .94, .024, dimensions.depth * .94]} /><meshStandardMaterial color="#969892" roughness={.96} metalness={0} /></mesh></group>; })}{speakers.map((speaker) => <PovSpeaker key={speaker.id} speaker={speaker} activity={activityBySpeaker[speaker.id] ?? 0} bandActivity={bandActivityBySpeaker[speaker.id]} centerY={stackResolver.getCenterMeters(speaker)} xy={stackResolver.getXY(speaker)} room={room} />)}</>; }
