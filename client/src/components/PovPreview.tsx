...
type Props = { speakers: ClubSpeaker[]; activityBySpeaker: Readonly<Record<string, number>>; lowActivityBySpeaker: Readonly<Record<string, number>>; bandActivityBySpeaker: SpeakerBandActivityMap; listener: ClubListener; surfaceTone: SurfaceTone; isPlaying: boolean; onLook: (deltaYaw: number, deltaPitch: number) => void; onLookAbsolute: (yaw: number, pitch: number) => void };
...
function PovSpeaker({ speaker, activity, bandActivity, centerY, xy }: { speaker: ClubSpeaker; activity: number; bandActivity: SpeakerBandActivityMap[string] | undefined; centerY: number; xy: { x: number; y: number } }) { const [x, , z] = world(xy); return <group position={[x, centerY, z]} rotation={[0, speaker.orientation?.yaw ?? 0, 0]}><SpeakerMiniature kind={speaker.kind} modelId={speaker.modelId} activity={activity} bandActivity={bandActivity} glowStrength={1.45} /></group>; }
function PovWorld({ speakers, activityBySpeaker, bandActivityBySpeaker, surfaceTone, isPlaying }: { speakers: ClubSpeaker[]; activityBySpeaker: Readonly<Record<string, number>>; bandActivityBySpeaker: SpeakerBandActivityMap; surfaceTone: SurfaceTone; isPlaying: boolean }) { const surface = povPalette[surfaceTone]; const stackResolver = createStackResolver(speakers); return <><fog attach="fog" args={[surface.fog, 8, 30]} /><ambientLight intensity={surface.ambient} /><directionalLight position={[-4, 8, 3]} intensity={surface.directional} color={surface.light} /><mesh rotation={[-Math.PI / 2, 0, 0]}><planeGeometry args={[28, 22]} /><meshStandardMaterial color={surface.floor} roughness={.96} /></mesh><group position={[0, 0, -3.5]}><DjBooth active={isPlaying} tone={surfaceTone} /></group>{speakers.map((speaker) => <PovSpeaker key={speaker.id} speaker={speaker} activity={activityBySpeaker[speaker.id] ?? 0} bandActivity={bandActivityBySpeaker[speaker.id]} centerY={stackResolver.getCenterMeters(speaker)} xy={stackResolver.getXY(speaker)} />)}<gridHelper args={[24, 24, surface.gridMajor, surface.gridMinor]} position={[0, .01, 0]} /></>; }

export default function PovPreview({ speakers, activityBySpeaker, lowActivityBySpeaker, bandActivityBySpeaker, listener, surfaceTone, isPlaying, onLook, onLookAbsolute }: Props) {
