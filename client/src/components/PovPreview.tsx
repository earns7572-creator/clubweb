...
function PovSpeaker({ speaker, activity, centerY, xy }: { speaker: ClubSpeaker; activity: number; centerY: number; xy: { x: number; y: number } }) { const [x, , z] = world(xy); return <group position={[x, centerY, z]}><SpeakerMiniature kind={speaker.kind} modelId={speaker.modelId} activity={activity} /></group>; }
