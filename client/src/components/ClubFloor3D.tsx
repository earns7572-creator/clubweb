...
function SpeakerObject({ speaker, activity, selected, canRemove, centerY, xy, onRemove, onSelect, onDragStart, onDragMove, onDragEnd, onRotateStart, onRotateMove, onRotateEnd }: { speaker: ClubSpeaker; activity: number; selected: boolean; canRemove: boolean; centerY: number; xy: Point; onRemove: () => void; onSelect: () => void; onDragStart: (event: ThreeEvent<PointerEvent>) => void; onDragMove: (event: ThreeEvent<PointerEvent>) => void; onDragEnd: (event: ThreeEvent<PointerEvent>) => void; onRotateStart: (event: ThreeEvent<PointerEvent>) => void; onRotateMove: (event: ThreeEvent<PointerEvent>) => void; onRotateEnd: (event: ThreeEvent<PointerEvent>) => void }) {
  ...
  return <group position={[x, centerY, z]} onPointerDown={(event) => { event.stopPropagation(); onSelect(); onDragStart(event); }} onPointerMove={(event) => { event.stopPropagation(); onDragMove(event); }} onPointerUp={(event) => { event.stopPropagation(); onDragEnd(event); }}>
    ...
    <group rotation={[0, speaker.orientation?.yaw ?? 0, 0]}><SpeakerMiniature kind={speaker.kind} modelId={speaker.modelId} activity={activity} selected={selected} idleVisible />{selected && <group position={[0, height / 2 + .05, depth / 2 + .30]} onPointerDown={(event) => { event.stopPropagation(); onRotateStart(event); }} onPointerMove={(event) => { event.stopPropagation(); onRotateMove(event); }} onPointerUp={(event) => { event.stopPropagation(); onRotateEnd(event); }}><mesh rotation={[-Math.PI / 2, 0, 0]}><circleGeometry args={[.34, 32]} /><meshBasicMaterial transparent opacity={0} depthWrite={false} /></mesh><mesh rotation={[-Math.PI / 2, 0, 0]}><ringGeometry args={[.23, .255, 32]} /><meshBasicMaterial color="#c6b48d" transparent opacity={.94} depthWrite={false} side={THREE.DoubleSide} /></mesh><mesh position={[0, -.012, -.27]}><boxGeometry args={[.038, .022, .34]} /><meshBasicMaterial color="#c6b48d" transparent opacity={.95} /></mesh><mesh position={[0, -.012, -.08]} rotation={[-Math.PI / 2, 0, 0]}><coneGeometry args={[.095, .16, 3]} /><meshBasicMaterial color="#e7d7ae" transparent opacity={1} /></mesh><Html position={[0, .04, .32]} center sprite pointerEvents="none"><span className="speaker-rotate-label">TURN</span></Html></group>}</group>
  </group>;
}
