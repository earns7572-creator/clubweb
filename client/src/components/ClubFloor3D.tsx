/**
 * Spatial Installation UI: a quiet continuous canvas with precise monochrome speaker miniatures.
...
function SpeakerObject({ speaker, selected, onSelect, onDragStart, onDragMove, onDragEnd }: { speaker: ClubSpeaker; selected: boolean; onSelect: () => void; onDragStart: (event: ThreeEvent<PointerEvent>) => void; onDragMove: (event: ThreeEvent<PointerEvent>) => void; onDragEnd: () => void }) {
  ...
  return <group position={[x, height / 2 + lift, z]} onPointerDown={(event) => { event.stopPropagation(); onSelect(); onDragStart(event); }} onPointerMove={(event) => { event.stopPropagation(); onDragMove(event); }} onPointerUp={(event) => { event.stopPropagation(); onDragEnd(); }}>
    ...
    {selected && <mesh position={[0, -height / 2 - lift + .012, 0]} rotation={[-Math.PI / 2, 0, 0]}><ringGeometry args={[Math.max(width, depth) * .7, Math.max(width, depth) * .715, 40]} /><meshBasicMaterial color="#50514b" transparent opacity={.38} side={THREE.DoubleSide} /></mesh>}
...
function ListenerObject({ listener, onDragStart, onDragMove, onDragEnd }: { listener: ClubListener; onDragStart: (event: ThreeEvent<PointerEvent>) => void; onDragMove: (event: ThreeEvent<PointerEvent>) => void; onDragEnd: () => void }) {
  ...
  return <group position={[x, .14, z]} onPointerDown={(event) => { event.stopPropagation(); onDragStart(event); }} onPointerMove={(event) => { event.stopPropagation(); onDragMove(event); }} onPointerUp={(event) => { event.stopPropagation(); onDragEnd(); }}>
    <mesh position={[0, -.126, 0]} rotation={[-Math.PI / 2, 0, 0]}><ringGeometry args={[.45, .46, 48]} /><meshBasicMaterial color="#777870" transparent opacity={.14} side={THREE.DoubleSide} /></mesh>
    <mesh rotation={[-Math.PI / 2, 0, 0]}><ringGeometry args={[.22, .228, 32]} /><meshBasicMaterial color="#50514b" transparent opacity={.38} side={THREE.DoubleSide} /></mesh>
...
function RoomScene(props: Props) {
  ...
  return <>
    ...
    <mesh rotation={[-Math.PI / 2, 0, 0]} receiveShadow onPointerMove={updateFromEvent} onPointerUp={stopDrag}><planeGeometry args={[roomWidth * 1.9, roomDepth * 1.9]} /><meshStandardMaterial color="#f6f5f1" roughness={.98} /></mesh>
    <Grid args={[roomWidth, roomDepth]} cellSize={roomWidth / 12} cellThickness={.18} cellColor="#d9d9d3" sectionSize={roomWidth} sectionThickness={.22} sectionColor="#c8c8c2" fadeDistance={30} fadeStrength={1.2} position={[0, .012, 0]} />
    ...
    <mesh position={[0, .02, -.7]} rotation={[-Math.PI / 2, 0, 0]}><ringGeometry args={[.22, .227, 36]} /><meshBasicMaterial color={props.signalActive ? "#d64b35" : "#50514b"} transparent opacity={props.signalActive ? .4 : .18} side={THREE.DoubleSide} /></mesh>
    <mesh position={[0, .045, -.7]}><sphereGeometry args={[.065, 20, 16]} /><meshStandardMaterial color={props.signalActive ? "#d64b35" : "#50514b"} roughness={.62} /></mesh>
    {selected && props.signalActive && <><mesh><tubeGeometry args={[new THREE.CatmullRomCurve3([sourcePoint, new THREE.Vector3(sourcePoint.x, .08, (sourcePoint.z + speakerPoint.z) / 2), speakerPoint]), 24, .007, 6, false]} /><meshBasicMaterial color="#d64b35" transparent opacity={.45} /></mesh><mesh><tubeGeometry args={[new THREE.CatmullRomCurve3([speakerPoint, new THREE.Vector3((speakerPoint.x + listenerPoint.x) / 2, .04, (speakerPoint.z + listenerPoint.z) / 2), listenerPoint]), 24, .006, 6, false]} /><meshBasicMaterial color="#d64b35" transparent opacity={.24} /></mesh></>}
...
export default function ClubFloor3D(props: Props) {
  return <div className="club-floor-3d"><Canvas orthographic shadows camera={{ position: [0, 14, 3.2], zoom: 92 }} dpr={[1, 1.5]} gl={{ antialias: true, alpha: true }}><color attach="background" args={["#f6f5f1"]} /><ResponsiveFloorCamera /><RoomScene {...props} /></Canvas><div className="spatial-stage-anchor"><i />stage</div><div className="spatial-scale-anchor"><i />0&nbsp;&nbsp;&nbsp;&nbsp;3&nbsp;&nbsp;&nbsp;&nbsp;6 m</div></div>;
}
