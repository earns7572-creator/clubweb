...
function SpeakerObject({ speaker, activity, selected, canRemove, centerY, xy, onRemove, onSelect, onDragStart, onDragMove, onDragEnd, onRotateStart, onRotateMove, onRotateEnd }: { speaker: ClubSpeaker; activity: number; selected: boolean; canRemove: boolean; centerY: number; xy: Point; onRemove: () => void; onSelect: () => void; onDragStart: (event: ThreeEvent<PointerEvent>) => void; onDragMove: (event: ThreeEvent<PointerEvent>) => void; onDragEnd: (event: ThreeEvent<PointerEvent>) => void; onRotateStart: (event: ThreeEvent<PointerEvent>) => void; onRotateMove: (event: ThreeEvent<PointerEvent>) => void; onRotateEnd: (event: ThreeEvent<PointerEvent>) => void }) {
  ...
  return <group position={[x, centerY, z]} onPointerDown={(event) => { event.stopPropagation(); onSelect(); onDragStart(event); }} onPointerMove={(event) => { event.stopPropagation(); onDragMove(event); }} onPointerUp={(event) => { event.stopPropagation(); onDragEnd(event); }}>
    ...
    <group rotation={[0, speaker.orientation?.yaw ?? 0, 0]}><SpeakerMiniature kind={speaker.kind} modelId={speaker.modelId} activity={activity} selected={selected} idleVisible />{selected && <group position={[0, height / 2 + .035, depth / 2 + .19]} onPointerDown={(event) => { event.stopPropagation(); onRotateStart(event); }} onPointerMove={(event) => { event.stopPropagation(); onRotateMove(event); }} onPointerUp={(event) => { event.stopPropagation(); onRotateEnd(event); }}><mesh rotation={[-Math.PI / 2, 0, 0]}><circleGeometry args={[.18, 24]} /><meshBasicMaterial transparent opacity={0} depthWrite={false} /></mesh><mesh position={[0, -.012, -.14]}><boxGeometry args={[.035, .018, .28]} /><meshBasicMaterial color="#827866" transparent opacity={.8} /></mesh><mesh position={[0, -.012, .02]} rotation={[-Math.PI / 2, 0, 0]}><coneGeometry args={[.07, .13, 3]} /><meshBasicMaterial color="#827866" transparent opacity={.9} /></mesh></group>}</group>
  </group>;
}
...
function RoomScene(props: Props) {
  ...
  const applyPending = () => {
    ...
    if (active.type === "rotation") { const speaker = stackResolver.byId.get(active.id); if (!speaker) return; const xy = stackResolver.getXY(speaker); const angle = Math.atan2(movement.raw.x - xy.x, movement.raw.y - xy.y); props.onSpeakerRotate(active.id, movement.modifiers.shift ? snapYaw(angle) : angle); invalidate(); return; }
