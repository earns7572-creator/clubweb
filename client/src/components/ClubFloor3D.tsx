...
import { createStackResolver, findStackAlignment, findStackCandidate, stackAlignmentPoint, type StackCandidate } from "@/lib/speakerStacking";
import { SimpleHumanAvatar } from "@/components/SimpleHumanAvatar";
import { snapYaw, yawToDegrees } from "@/lib/speakerOrientation";
import { getSpeakerModel } from "@/lib/speakerModels";
import SoundFieldLayer from "@/components/SoundFieldLayer";
import { clampStackRootPoint, exceedsDragThreshold, ROOM_LAYOUT_BOUNDS, resolveStackRootId } from "@/lib/speakerInteraction";
import type { SpeakerBandActivityMap } from "@/lib/bandActivity";

type Point = { x: number; y: number };
type SpeakerDrag = { type: "speaker"; id: string; rootId: string; offset: Point; snap: SmartSnapState; candidateParentId: string | null; candidate: StackCandidate | null; lastPoint: Point | null; startedAt: Point; pointerType: string; didMove: boolean };
...
type Props = { speakers: ClubSpeaker[]; activityBySpeaker: Readonly<Record<string, number>>; bandActivityBySpeaker: SpeakerBandActivityMap; listener: ClubListener; selectedSpeakerId: string; sourceColor?: string; signalActive?: boolean; surfaceTone: SurfaceTone; canRemove: boolean; onSpeakerSelect: (id: string) => void; onSpeakerRemove: (id: string) => void; onSpeakerMove: (id: string, position: Point) => void; onSpeakerRotate: (id: string, yaw: number) => void; onSpeakerStack: (id: string, parentId: string, alignment: StackCandidate["alignment"]) => void; onListenerMove: (position: Point) => void; onListenerNameChange: (name: string) => void; onFloorPlace?: (point: Point) => void };
...
function SpeakerObject({ speaker, activity, bandActivity, selected, centerY, xy, onSelect, onDragStart, onDragMove, onDragEnd }: { speaker: ClubSpeaker; activity: number; bandActivity: SpeakerBandActivityMap[string] | undefined; selected: boolean; centerY: number; xy: Point; onSelect: () => void; onDragStart: (event: ThreeEvent<PointerEvent>) => void; onDragMove: (event: ThreeEvent<PointerEvent>) => void; onDragEnd: (event: ThreeEvent<PointerEvent>) => void }) {
  const modelBody = getSpeakerModel(speaker.modelId, speaker.kind).body; const [width, height, depth] = [modelBody.width, modelBody.height, modelBody.depth]; const yaw = speaker.orientation?.yaw ?? 0;
  ...
  return <group position={[x, centerY, z]} onPointerDown={(event) => { event.stopPropagation(); onSelect(); onDragStart(event); }} onPointerMove={(event) => { event.stopPropagation(); onDragMove(event); }} onPointerUp={(event) => { event.stopPropagation(); onDragEnd(event); }}>
    <group rotation={[0, yaw, 0]}><mesh><boxGeometry args={[width, height, depth]} /><meshBasicMaterial transparent opacity={0} depthWrite={false} /></mesh>{selected && <mesh position={[0, -height / 2 + .012, 0]} rotation={[-Math.PI / 2, 0, 0]}><ringGeometry args={[Math.max(width, depth) * .7, Math.max(width, depth) * .715, 40]} /><meshBasicMaterial color="#c6c3b8" transparent opacity={.15} side={THREE.DoubleSide} /></mesh>}<SpeakerMiniature kind={speaker.kind} modelId={speaker.modelId} activity={activity} bandActivity={bandActivity} selected={selected} idleVisible glowStrength={.90} /></group>
  </group>;
}
...
function TurnControl({ speaker, onRotate }: { speaker: ClubSpeaker | undefined; onRotate: (id: string, yaw: number) => void }) {
  if (!speaker) return null;
  const yaw = speaker.orientation?.yaw ?? 0;
  const turn = (degrees: number) => onRotate(speaker.id, snapYaw(yaw + degrees * Math.PI / 180));
  return <div className="speaker-turn-control" aria-label="Speaker rotation"><span>TURN</span><output>{yawToDegrees(yaw)}°</output><button type="button" onClick={() => turn(-15)} aria-label="Turn speaker left 15 degrees">−15°</button><button type="button" onClick={() => turn(15)} aria-label="Turn speaker right 15 degrees">+15°</button></div>;
}
...
function RoomScene(props: Props) {
  ...
  const applyPending = () => {
    ...
    if (!active.didMove) { if (!exceedsDragThreshold(active.startedAt, movement.screen, active.pointerType)) return; active.didMove = true; }
    const dragged = stackResolver.byId.get(active.rootId); if (!dragged) return; const safeRaw = clampStackRootPoint(props.speakers, active.rootId, raw); active.lastPoint = safeRaw;
    ...
    active.snap = result.snap; setGuides(result.guides); props.onSpeakerMove(active.rootId, clampStackRootPoint(props.speakers, active.rootId, result.point)); invalidate();
  };
  ...
  const stop = (event: ThreeEvent<PointerEvent>) => { if (frame.current !== null) { cancelAnimationFrame(frame.current); applyPending(); } const active = drag.current; if (active?.type === "speaker" && active.didMove && active.candidate) props.onSpeakerStack(active.rootId, active.candidate.parentId, active.candidate.alignment); pointerTarget(event)?.releasePointerCapture?.(event.pointerId); pending.current = null; drag.current = null; setGuides(null); setStackCandidate(null); invalidate(); };
  useEffect(() => () => { if (frame.current !== null) cancelAnimationFrame(frame.current); }, []);
  const activeSpeaker = drag.current?.type === "speaker" ? stackResolver.byId.get(drag.current.id) : undefined;
  return <><ambientLight intensity={1.08} /><hemisphereLight args={[surface.sky, surface.ground, .82]} /><directionalLight position={[4, 9, 5]} intensity={1.12} color="#fffaf0" /><mesh rotation={[-Math.PI / 2, 0, 0]} onPointerDown={() => props.onSpeakerSelect("")} onPointerMove={move} onPointerUp={stop}><planeGeometry args={[roomWidth * 1.9, roomDepth * 1.9]} /><meshStandardMaterial color={surface.floor} roughness={.98} /></mesh><SoundFieldLayer speakers={props.speakers} bandActivityBySpeaker={props.bandActivityBySpeaker} roomWidth={roomWidth} roomDepth={roomDepth} hazeColor={surface.majorGrid} darkSurface={props.surfaceTone === "night"} /><Grid args={[roomWidth, roomDepth]} cellSize={.5} cellThickness={.07} cellColor={surface.minorGrid} sectionSize={1} sectionThickness={.18} sectionColor={surface.majorGrid} fadeDistance={30} fadeStrength={1.2} position={[0, .012, 0]} /><SmartGuides guides={guides} listener={props.listener} /><StackPreview candidate={stackCandidate} dragged={activeSpeaker} resolver={stackResolver} />{props.speakers.map((speaker) => <SpeakerObject key={speaker.id} speaker={speaker} activity={props.activityBySpeaker[speaker.id] ?? 0} bandActivity={props.bandActivityBySpeaker[speaker.id]} selected={speaker.id === props.selectedSpeakerId} centerY={stackResolver.getCenterMeters(speaker)} xy={stackResolver.getXY(speaker)} onSelect={() => props.onSpeakerSelect(speaker.id)} onDragStart={(event) => { const point = pointerPointOnFloor(event); if (!point) return; const rootId = resolveStackRootId(props.speakers, speaker.id); const root = stackResolver.byId.get(rootId); if (!root) return; const rootXY = stackResolver.getXY(root); start({ type: "speaker", id: speaker.id, rootId, offset: { x: rootXY.x - point.x, y: rootXY.y - point.y }, snap: {}, candidateParentId: null, candidate: null, lastPoint: null, startedAt: { x: event.nativeEvent.clientX, y: event.nativeEvent.clientY }, pointerType: event.nativeEvent.pointerType, didMove: false }, event); }} onDragMove={move} onDragEnd={stop} />)}<ListenerObject listener={props.listener} onNameChange={props.onListenerNameChange} onDragStart={(event) => { const point = pointerPointOnFloor(event); if (!point) return; start({ type: "listener", offset: { x: props.listener.position.x - point.x, y: props.listener.position.y - point.y } }, event); }} onDragMove={move} onDragEnd={stop} /></>;
}

export default function ClubFloor3D(props: Props) { const surface = surfacePalette[props.surfaceTone]; const selected = props.speakers.find((speaker) => speaker.id === props.selectedSpeakerId); return <div className="club-floor-3d"><TurnControl speaker={selected} onRotate={props.onSpeakerRotate} /><Canvas frameloop="demand" orthographic camera={{ position: [0, 14, 3.2], zoom: 108 }} dpr={[1, 1.25]} gl={{ antialias: true, alpha: true }}><color attach="background" args={[surface.background]} /><ResponsiveFloorCamera /><RoomScene {...props} /></Canvas></div>; }
