...
function SpeakerObject({ speaker, activity, bandActivity, selected, dragging = false, centerY, xy, canRemove, worldPerPixel, mobile, onSelect, onRemove, onRotate, onDragStart, onDragMove, onDragEnd }: { speaker: ClubSpeaker; activity: number; bandActivity: SpeakerBandActivityMap[string] | undefined; selected: boolean; dragging?: boolean; centerY: number; xy: Point; canRemove: boolean; worldPerPixel: number; mobile: boolean; onSelect: () => void; onRemove: () => void; onRotate: (id: string, yaw: number) => void; onDragStart: (event: ThreeEvent<PointerEvent>) => void; onDragMove: (event: ThreeEvent<PointerEvent>) => void; onDragEnd: (event: ThreeEvent<PointerEvent>) => void }) {
  ...
  const { invalidate } = useThree(); const visualRef = useRef<THREE.Group>(null); const targetScale = dragging ? 1.03 : 1;
  useFrame((_, delta) => { const visual = visualRef.current; if (!visual) return; const nextScale = THREE.MathUtils.damp(visual.scale.x, targetScale, dragging ? 28 : 11, delta); visual.scale.setScalar(nextScale); if (Math.abs(nextScale - targetScale) > .001) invalidate(); });
...
function ResponsiveFloorCamera() { const { camera, size, invalidate } = useThree(); useEffect(() => { const orthographic = camera as THREE.OrthographicCamera; orthographic.position.set(0, 14, 3.2); orthographic.zoom = size.width < 760 ? 52 : 108; // Release framing leaves negative space around the objects without changing room or cabinet dimensions.
    orthographic.zoom *= size.width < 760 ? .80 : .78; orthographic.lookAt(0, 0, 0); orthographic.updateProjectionMatrix(); invalidate(); }, [camera, invalidate, size.width, size.height]); return null; }
