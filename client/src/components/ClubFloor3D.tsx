...
function RoomScene(props: Props) {
  ...
  const stop = (event: ThreeEvent<PointerEvent>) => { if (frame.current !== null) { cancelAnimationFrame(frame.current); applyPending(); } const active = drag.current; if (active?.type === "speaker") { if (active.didMove && active.candidate) props.onSpeakerStack(active.rootId, active.candidate.parentId, active.candidate.alignment); if (!active.didMove) props.onSpeakerSelect(active.id); } pointerTarget(event)?.releasePointerCapture?.(event.pointerId); pending.current = null; drag.current = null; setGuides(null); setStackCandidate(null); invalidate(); };
