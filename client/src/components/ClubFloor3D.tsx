...
type Props = { speakers: ClubSpeaker[]; activityBySpeaker: Readonly<Record<string, number>>; listener: ClubListener; selectedSpeakerId: string; sourceColor?: string; signalActive?: boolean; canRemove: boolean; onSpeakerSelect: (id: string) => void; onSpeakerRemove: (id: string) => void; onSpeakerMove: (id: string, position: Point) => void; onListenerMove: (position: Point) => void };
...
function SpeakerObject({ speaker, activity, selected, canRemove, onRemove, onSelect, onDragStart, onDragMove, onDragEnd }: { speaker: ClubSpeaker; activity: number; selected: boolean; canRemove: boolean; onRemove: () => void; onSelect: () => void; onDragStart: (event: ThreeEvent<PointerEvent>) => void; onDragMove: (event: ThreeEvent<PointerEvent>) => void; onDragEnd: (event: ThreeEvent<PointerEvent>) => void }) {
  ...
  return <group position={[x, height / 2 + lift, z]} onPointerDown={(event) => { event.stopPropagation(); onSelect(); onDragStart(event); }} onPointerMove={(event) => { event.stopPropagation(); onDragMove(event); }} onPointerUp={(event) => { event.stopPropagation(); onDragEnd(event); }}>
    ...
    <SpeakerMiniature kind={speaker.kind} activity={activity} selected={selected} idleVisible />
  </group>;
}
...
function RoomScene(props: Props) {
  const { camera, size, invalidate } = useThree(); const drag = useRef<DragTarget>(null); const pending = useRef<PendingDrag>(null); const frame = useRef<number | null>(null); const [guides, setGuides] = useState<SmartGuideState | null>(null);
  ...
  const applyPending = () => {
    ...
    active.snap = result.snap; setGuides(result.guides); props.onSpeakerMove(active.id, result.point); invalidate();
  };
  const queue = () => { if (frame.current === null) frame.current = requestAnimationFrame(applyPending); };
  const start = (target: DragTarget, event: ThreeEvent<PointerEvent>) => { pointerTarget(event)?.setPointerCapture?.(event.pointerId); pending.current = null; setGuides(null); drag.current = target; invalidate(); };
  ...
  const stop = (event: ThreeEvent<PointerEvent>) => { if (frame.current !== null) { cancelAnimationFrame(frame.current); applyPending(); } pointerTarget(event)?.releasePointerCapture?.(event.pointerId); pending.current = null; drag.current = null; setGuides(null); invalidate(); };
  useEffect(() => () => { if (frame.current !== null) cancelAnimationFrame(frame.current); }, []);
  return <><ambientLight intensity={1.08} /><hemisphereLight args={["#f4f1e8", "#9e9f98", .82]} /><directionalLight position={[4, 9, 5]} intensity={1.12} color="#fffaf0" /><mesh rotation={[-Math.PI / 2, 0, 0]} onPointerMove={move} onPointerUp={stop}><planeGeometry args={[roomWidth * 1.9, roomDepth * 1.9]} /><meshStandardMaterial color="#e9e7df" roughness={.98} /></mesh><Grid args={[roomWidth, roomDepth]} cellSize={.5} cellThickness={.07} cellColor="#d4d2c9" sectionSize={1} sectionThickness={.13} sectionColor="#aaa9a0" fadeDistance={30} fadeStrength={1.2} position={[0, .012, 0]} /><group position={[0, .09, -3.3]}><mesh><boxGeometry args={[1.75, .18, .55]} /><meshStandardMaterial color="#706f68" roughness={.86} /></mesh><mesh position={[0, .095, 0]}><boxGeometry args={[1.45, .014, .34]} /><meshStandardMaterial color="#cbc7ba" roughness={.9} /></mesh></group><mesh position={[0, .02, -.7]} rotation={[-Math.PI / 2, 0, 0]}><ringGeometry args={[.22, .227, 36]} /><meshBasicMaterial color={props.signalActive ? "#d64b35" : "#8a8982"} transparent opacity={.35} side={THREE.DoubleSide} /></mesh><mesh position={[0, .045, -.7]}><sphereGeometry args={[.065, 20, 16]} /><meshStandardMaterial color={props.signalActive ? "#d64b35" : "#777770"} emissive={props.signalActive ? "#8f2e22" : "#393a35"} emissiveIntensity={props.signalActive ? .3 : .2} roughness={.62} /></mesh><SmartGuides guides={guides} listener={props.listener} />{props.speakers.map((speaker) => <SpeakerObject key={speaker.id} speaker={speaker} activity={props.activityBySpeaker[speaker.id] ?? 0} selected={speaker.id === props.selectedSpeakerId} canRemove={props.canRemove} onRemove={() => props.onSpeakerRemove(speaker.id)} onSelect={() => props.onSpeakerSelect(speaker.id)} onDragStart={(event) => { const point = pointerPointOnFloor(event); if (!point) return; start({ type: "speaker", id: speaker.id, offset: { x: speaker.position.x - point.x, y: speaker.position.y - point.y }, snap: {} }, event); }} onDragMove={move} onDragEnd={stop} />)}<ListenerObject listener={props.listener} onDragStart={(event) => { const point = pointerPointOnFloor(event); if (!point) return; start({ type: "listener", offset: { x: props.listener.position.x - point.x, y: props.listener.position.y - point.y } }, event); }} onDragMove={move} onDragEnd={stop} /></>;
}

export default function ClubFloor3D(props: Props) { return <div className="club-floor-3d"><Canvas frameloop="demand" orthographic camera={{ position: [0, 14, 3.2], zoom: 92 }} dpr={[1, 1.25]} gl={{ antialias: true, alpha: true }}><color attach="background" args={["#f6f4ee"]} /><ResponsiveFloorCamera /><RoomScene {...props} /></Canvas><div className="spatial-stage-anchor"><i />stage</div><div className="spatial-scale-anchor"><i />0&nbsp;&nbsp;&nbsp;&nbsp;3&nbsp;&nbsp;&nbsp;&nbsp;6 m</div></div>; }
