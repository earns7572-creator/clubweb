...
type Props = { speakers: ClubSpeaker[]; activityBySpeaker: Readonly<Record<string, number>>; listener: ClubListener; selectedSpeakerId: string; sourceColor?: string; signalActive?: boolean; surfaceTone: SurfaceTone; canRemove: boolean; onSpeakerSelect: (id: string) => void; onSpeakerRemove: (id: string) => void; onSpeakerMove: (id: string, position: Point) => void; onSpeakerStack: (id: string, parentId: string) => void; onSpeakerDetach: (id: string, position: Point) => void; onListenerMove: (position: Point) => void };
...
function SpeakerObject({ speaker, activity, selected, canRemove, centerY, xy, onRemove, onSelect, onDragStart, onDragMove, onDragEnd }: { speaker: ClubSpeaker; activity: number; selected: boolean; canRemove: boolean; centerY: number; xy: Point; onRemove: () => void; onSelect: () => void; onDragStart: (event: ThreeEvent<PointerEvent>) => void; onDragMove: (event: ThreeEvent<PointerEvent>) => void; onDragEnd: (event: ThreeEvent<PointerEvent>) => void }) {
  const [width, height, depth] = speakerBlueprints[speaker.kind].body;
...
function RoomScene(props: Props) {
  ...
  const applyPending = () => {
    frame.current = null; const active = drag.current; const movement = pending.current; pending.current = null; if (!active || !movement) return; const raw = { x: movement.raw.x + active.offset.x, y: movement.raw.y + active.offset.y };
