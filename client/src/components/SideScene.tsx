...
type Props = { speakers: ClubSpeaker[]; listener: ClubListener; selectedSpeakerId: string; canRemove: boolean; onSpeakerSelect: (id: string) => void; onSpeakerRemove: (id: string) => void; onSpeakerMove: (id: string, position: { y: number; z: number }) => void };
const clamp = (value: number) => Math.max(.04, Math.min(.96, value));

export default function SideScene({ speakers, listener, selectedSpeakerId, canRemove, onSpeakerSelect, onSpeakerRemove, onSpeakerMove }: Props) {
  ...
  return <div ref={sceneRef} className="side-scene" onPointerMove={update} onPointerUp={() => { dragRef.current = null; }} onPointerLeave={() => { dragRef.current = null; }}>
    ...
    {speakers.map((speaker, index) => <div key={speaker.id} className={`side-speaker ${speaker.kind} ${speaker.id === selectedSpeakerId ? "selected" : ""}`} style={{ left: `${speaker.position.y * 88 + 6}%`, bottom: `${speaker.position.z * 74 + 9}%`, marginLeft: `${((index % 3) - 1) * 2}px`, "--activity": speaker.activity } as React.CSSProperties}><button className="side-speaker-drag" onPointerDown={(event) => { event.preventDefault(); event.stopPropagation(); dragRef.current = speaker.id; onSpeakerSelect(speaker.id); sceneRef.current?.setPointerCapture(event.pointerId); }} aria-label={`Move ${speaker.kind} speaker`}><i /><b /></button>{speaker.id === selectedSpeakerId && canRemove && <button className="side-cabinet-remove" onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); onSpeakerRemove(speaker.id); }} aria-label={`Remove ${speaker.kind} speaker`}>×</button>}</div>)}
