  ...
  return <div className={`first-use-onboarding first-use-${step}`} aria-live="polite" onPointerDown={(event) => { if (step !== "place" || event.target !== event.currentTarget) return; const bounds = event.currentTarget.getBoundingClientRect(); onPlace({ x: (event.clientX - bounds.left) / bounds.width, y: (event.clientY - bounds.top) / bounds.height }); }}>
    <section className="first-use-panel">
      ...
      {step === "speaker" && <>
        ...
        <div className="first-use-model-grid">{modelIdsForFamily(family).map((modelId) => { const model = getSpeakerModel(modelId, "sub"); return <button key={modelId} className={`first-use-model-card ${model.family} ${model.kind}`} onClick={() => onModelChoose(modelId)}><i aria-hidden="true" /><span><strong>{model.shortLabel}</strong><small>{bandLabel[model.kind]}</small></span></button>; })}</div>
      </>}
