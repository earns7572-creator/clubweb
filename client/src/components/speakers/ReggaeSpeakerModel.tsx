...
function Scoop({ activity, glowStrength, cabinetColor }: { activity: number; glowStrength: number; cabinetColor?: string }) {
  const materials = useScoopMaterials(cabinetColor); const body = getSpeakerModel("reggae-scoop", "sub").body; const front = body.depth / 2 - .14; const rail = .075; const upperHeight = .67; const mouthHeight = .86; const wooferY = .39; const mouthY = -.42; const scoopDepth = .56; const innerWidth = body.width - rail * 2;
  return <group>
...
function Kick({ activity, glowStrength, cabinetColor }: { activity: number; glowStrength: number; cabinetColor?: string }) {
  const materials = useScoopMaterials(cabinetColor); const body = getSpeakerModel("reggae-kick", "woofer").body; const front = body.depth / 2 - .1; const rail = .06; const innerWidth = body.width - rail * 2; const wooferY = .1; const portY = -.32; const portHeight = .15;
  return <group>
...
function MidHorn({ activity, glowStrength, cabinetColor }: { activity: number; glowStrength: number; cabinetColor?: string }) {
  const materials = useScoopMaterials(cabinetColor); const body = getSpeakerModel("reggae-mid-horn", "mid").body; const front = body.depth / 2 - .055; const rail = .065; const innerWidth = body.width - rail * 2;
  return <group>
    <mesh geometry={box} position={[0, 0, -body.depth / 2 + .035]} scale={[body.width, body.height, .07]} material={materials.cabinet} />
    <mesh geometry={box} position={[-body.width / 2 + .035, 0, 0]} scale={[.07, body.height, body.depth]} material={materials.cabinet} />
    <mesh geometry={box} position={[body.width / 2 - .035, 0, 0]} scale={[.07, body.height, body.depth]} material={materials.cabinet} />
...
function Top({ activity, glowStrength, cabinetColor }: { activity: number; glowStrength: number; cabinetColor?: string }) {
  const materials = useScoopMaterials(cabinetColor); const body = getSpeakerModel("reggae-top", "high").body; const front = body.depth / 2 - .045; const rail = .06; const hornWidth = .41; const hornHeight = .19; const throatZ = front - .245;
  return <group>
...
export default function ReggaeSpeakerModel({ modelId, kind, activity, bandActivity, selected = false, glowStrength = .8, cabinetColor }: Props) { const model = getSpeakerModel(modelId, kind); const bands = bandActivity ?? { overall: activity, low: activity, mid: activity, high: activity }; return <group>{modelId === "reggae-scoop" && <Scoop activity={bands.low} glowStrength={glowStrength} cabinetColor={cabinetColor} />}{modelId === "reggae-kick" && <Kick activity={bands.low} glowStrength={glowStrength} cabinetColor={cabinetColor} />}{modelId === "reggae-mid-horn" && <MidHorn activity={bands.mid} glowStrength={glowStrength} cabinetColor={cabinetColor} />}{modelId === "reggae-top" && <Top activity={bands.high} glowStrength={glowStrength} cabinetColor={cabinetColor} />}{selected && <mesh geometry={box} scale={[model.body.width * 1.018, model.body.height * 1.018, model.body.depth * 1.018]} material={selection} />}</group>; }
