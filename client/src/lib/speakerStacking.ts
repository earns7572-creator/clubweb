...
import { DESKTOP_ROOM_METERS, type RoomMetrics } from "@/lib/roomGeometry";

export const STACK_ROOM_METERS = DESKTOP_ROOM_METERS;
...
export function stackAlignmentPoint(parent: ClubSpeaker, child: ClubSpeaker, parentPoint: StackPoint, alignment: StackAlignment, room: RoomMetrics = STACK_ROOM_METERS): StackPoint {
  ...
  return {
    x: parentPoint.x + (Math.cos(yaw) * lateralOffset + Math.sin(yaw) * frontOffset) / room.width,
    y: parentPoint.y + (-Math.sin(yaw) * lateralOffset + Math.cos(yaw) * frontOffset) / room.depth,
  };
}
...
export function createStackResolver(speakers: ClubSpeaker[], room: RoomMetrics = STACK_ROOM_METERS) {
...
export function mobileStackTargetMeters(worldPerPixel: number, mobile: boolean) {
  return mobile ? Math.max(.45, Math.min(.9, worldPerPixel * 72)) : 0;
}

export function findStackCandidate({ dragged, point, speakers, previousParentId, minimumTargetMeters = 0, room = STACK_ROOM_METERS }: { dragged: ClubSpeaker; point: StackPoint; speakers: ClubSpeaker[]; previousParentId?: string | null; minimumTargetMeters?: number; room?: RoomMetrics }) {
  const resolver = createStackResolver(speakers, room); const draggedBody = speakerBodyForSpeaker(dragged); let best: StackCandidate | null = null;
  for (const target of speakers) { const topId = resolver.getStackTop(target.id); if (target.id !== topId || topId === dragged.id || resolver.isDescendant(topId, dragged.id)) continue; const top = resolver.byId.get(topId); if (!top) continue; const xy = resolver.getXY(top); const targetBody = speakerBodyForSpeaker(top); const factor = previousParentId === topId ? STACK_RELEASE_FACTOR : STACK_ENTER_FACTOR; const thresholdX = Math.max(Math.max(draggedBody.width, targetBody.width) * factor, minimumTargetMeters) / room.width; const thresholdY = Math.max(Math.max(draggedBody.depth, targetBody.depth) * factor, minimumTargetMeters) / room.depth; const dx = Math.abs(point.x - xy.x); const dy = Math.abs(point.y - xy.y); if (dx > thresholdX || dy > thresholdY) continue; for (const alignment of ["left", "center", "right"] as const) { const anchor = stackAlignmentPoint(top, dragged, xy, alignment, room); const score = Math.hypot((point.x - anchor.x) / thresholdX, (point.y - anchor.y) / thresholdY); if (!best || score < best.score) best = { parentId: topId, alignment, score }; } }
  return best;
}

export function isWithinStackFootprint(speaker: ClubSpeaker, point: StackPoint, speakers: ClubSpeaker[], factor = STACK_RELEASE_FACTOR, room: RoomMetrics = STACK_ROOM_METERS) { const resolver = createStackResolver(speakers, room); const xy = resolver.getXY(speaker); const body = speakerBodyForSpeaker(speaker); return Math.abs(point.x - xy.x) <= body.width / room.width * factor && Math.abs(point.y - xy.y) <= body.depth / room.depth * factor; }
