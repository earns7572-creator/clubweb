...
const frontCenter = (parent, child, parentPoint, childPoint) => {
  ...
  const depths = { sub: 1.4, woofer: 1.08, full: .65, mid: .58, high: .72 };
...
assert.equal(resolver.getCenterMeters(full), 1.44, "full center follows physical body height");
assert.equal(resolver.getBottomMeters(high), 1.98, "third cabinet rests on compact full-range body");
assert.equal(resolver.getCenterMeters(high), 2.29, "high center uses its own body height");
close(resolver.getXY(full).y, .546875, "same-yaw child front flushes to a deeper parent");
