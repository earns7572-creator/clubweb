...
const frontCenter = (parent, child, parentPoint, childPoint) => {
  ...
  const depths = { sub: 1.4, woofer: 1.08, full: .58, mid: .58, high: .72 };
...
close(resolver.getCenterMeters(full), 1.59, "full center follows physical body height");
assert.equal(resolver.getBottomMeters(high), 2.28, "third cabinet rests on tall dual-woofer full-range body");
assert.equal(resolver.getCenterMeters(high), 2.59, "high center uses its own body height");
close(resolver.getXY(full).y, .55125, "same-yaw child front flushes to a deeper parent");
