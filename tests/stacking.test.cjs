...
const frontCenter = (parent, child, parentPoint, childPoint) => {
  ...
  const depths = { sub: 1.4, woofer: 1.08, full: .7, mid: .58, high: .72 };
...
close(resolver.getCenterMeters(full), 1.66, "full center follows physical body height");
assert.equal(resolver.getBottomMeters(high), 2.42, "third cabinet rests on dual-woofer full-range body");
assert.equal(resolver.getCenterMeters(high), 2.73, "high center uses its own body height");
close(resolver.getXY(full).y, .54375, "same-yaw child front flushes to a deeper parent");
