...
assert.equal(parsed.speakers.length, 2); assert.equal(parsed.speakers[1].modelId, "freeparty-mid-horn"); assert.equal(parsed.speakers[0].x, .31); assert.equal(parsed.speakers[0].y, .42); assert.equal(parsed.speakers[0].z, .13); assert.equal(parsed.speakers[0].yaw, .26); assert.equal(parsed.speakers[1].stackOn, "speaker-01"); assert.deepEqual(parsed.listener, { x: .55, y: .66, z: .22, yaw: .4, pitch: -.2 });
const restored = layoutToClubSpeakers(parsed, 123); assert.equal(restored[1].stackParentId, restored[0].id); assert.equal(restored[1].orientation?.yaw, -.26);
