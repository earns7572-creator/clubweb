...
assert.match(miniature, /function WooferAssembly[\s\S]*frontZ \+ \(recessed \? 0 : \.015\)/);
...
assert.match(home, /COMPONENTS/);
assert.doesNotMatch(home, /prepareRecipeMaterials|materialStaging/);
...
assert.match(home, /const selectSpeaker = \(id: string\) => \{ setSelectedSpeakerId\(id\); setMobileInspectorOpen\(false\); setSpeakerTrayOpen\(false\); setActiveHeaderPopover\(null\); \}/);
assert.match(home, /<ObjectControlsAnchor/);
assert.match(home, /open=\{mobileInspectorOpen\} hidden=\{inspectorHidden\}/);
assert.match(home, /className="mobile-speaker-edit" onClick=\{openInspector\}/);
assert.match(home, /const openInspector = \(\) => \{ setSpeakerTrayOpen\(false\); setActiveHeaderPopover\(null\); setMobileInspectorOpen\(true\); \}/);
