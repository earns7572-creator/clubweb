...
assert.match(audio, /const directivity = getSpeakerModel\(modelId, "sub"\)\.directivity/);
assert.match(audio, /panner\.distanceModel = "inverse"; panner\.refDistance = 1\.2; panner\.maxDistance = 12; panner\.rolloffFactor = \.85; panner\.coneInnerAngle = directivity\.innerAngle; panner\.coneOuterAngle = directivity\.outerAngle; panner\.coneOuterGain = directivity\.outerGain/);
...
assert.match(deviceLook, /relativeQuaternion\.current\.copy\(baselineQuaternion\.current\)\.invert\(\)\.multiply\(currentQuaternion\.current\)/);
